...
import {
  BookOpen, Award, Activity, CheckCircle, Download,
...
function AnimCount({ to, duration = 800 }) {
  ...
  useEffect(() => {
    ...
    const step = (ts) => {
      ...
      setV(Math.round(n * (1 - Math.pow(1 - p, 3)))); // ease-out cubic
...
function downloadCertificatePNG(enrollment, user) {
  ...
  ctx.fillStyle = "#c4b5fd"; ctx.font = "bold 20px Arial"; ctx.textAlign = "center";
  ...
  ctx.fillText(enrollment.course?.title || "Course", W / 2, 558, W - 200);
  ...
  a.download = `${((enrollment.course?.title) || "Certificate").replace(/\s+/g, "_")}_Certificate.png`;
  a.href = canvas.toDataURL("image/png", 1.0); a.click();
}

/* ─── Full-screen certificate modal (portal) ──────────────────────────────── */
function CertificateModal({ enrollment, user, onClose }) {
  ...
  return createPortal(
    <motion.div
      ...
      onClick={onClose}
    >
      {/* Scrollable inner — pt-20 clears the fixed navbar, pb-10 gives breathing room */}
      <div className="min-h-full flex flex-col items-center pt-20 pb-10 px-4">
        <motion.div
        ...
        <div className="relative bg-gradient-to-br from-[#0d0825] to-[#180a30] border-4 border-violet-600 rounded-2xl p-10 overflow-hidden shadow-[0_0_80px_rgba(124,58,237,0.35)]">
          ...
          <div className="relative text-center py-6">
            ...
            <p className="text-2xl sm:text-3xl font-black text-white mb-2 px-4">{enrollment.course?.title}</p>
...
export default function StudentDashboard({ activeTab }) {
  ...
  useEffect(() => {
    ...
    const stored = parseInt(localStorage.getItem("lms_streak") || "0") || 0;
    if (!lastVisit) {
      setStreak(1);
  ...
  // Guard against undefined progress (|| 0) to prevent NaN
  const avgCompletion = enrollments.length
    ? Math.round(enrollments.reduce((sum, e) => sum + (e.progress || 0), 0) / enrollments.length)
    : 0;

  // XP & Level (guard against NaN)
  const rawXp = enrollments.length * 10 + completedCount * 100 + avgCompletion + streak * 5;
  const xp = isNaN(rawXp) ? 0 : rawXp;
  ...
  const cardHover = { whileHover: { y: -3, scale: 1.01 }, whileTap: { scale: 0.98 }, transition: { type: "spring", stiffness: 350, damping: 20 } };

  return (
    <>
      <AnimatePresence>
        {selectedCert && (
          <CertificateModal enrollment={selectedCert} user={user} onClose={() => setSelectedCert(null)} />
        )}
      </AnimatePresence>
      ...
      {activeTab === "overview" && (
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
            {[
              { label: "Enrolled",     value: enrollments.length, icon: BookOpen,    color: "text-blue-400",    bg: "bg-blue-500/10",    border: "border-blue-500/20"    },
              { label: "In Progress",  value: inProgress,         icon: Activity,    color: "text-violet-400",  bg: "bg-violet-500/10",  border: "border-violet-500/20"  },
              ...
              { label: "Certificates", value: completedCount,     icon: Award,       color: "text-amber-400",   bg: "bg-amber-500/10",   border: "border-amber-500/20"   },
              ...
              <motion.div
                ...
                transition={{ ...cardHover.transition, delay: i * 0.06 }}
                className={`bg-[#120B24] p-5 rounded-2xl border ${stat.border} cursor-default`}
              >
                <div className={`w-10 h-10 rounded-xl mb-3 flex items-center justify-center ${stat.bg} border ${stat.border}`}>
                  <stat.icon className={`h-5 w-5 ${stat.color}`} />
                </div>
                <h3 className={`text-3xl font-bold text-white ${stat.color === "text-orange-400" && stat.value === 0 ? "opacity-40" : ""}`}>
                  <AnimCount to={stat.value} />
                </h3>
          ...
          <motion.div
            ...
            className={`bg-[#120B24] border ${level.border} rounded-2xl p-5 mb-8`}
          >
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-xs font-medium"><AnimCount to={xp} /> XP earned</p>
                </div>
              </div>
              <div className="text-right">
                ...
                {level.max !== 9999 && (
                  <p className="text-slate-500 text-xs">
                    {level.max - xp > 0 ? `${level.max - xp} XP to next` : "Max level!"}
                  </p>
                )}
              </div>
            </div>
            <div className="w-full bg-[#080410] rounded-full h-3 border border-[#2A1B4E] overflow-hidden">
              <motion.div
                ...
                <div className="space-y-2 flex-1">
                  {enrollments.slice(0, 3).map((e) => (
                    <motion.div
                      ...
                      className="flex items-center gap-3 p-3 bg-[#080410] rounded-xl border border-[#2A1B4E] hover:border-violet-500/30 transition-colors cursor-pointer"
                    >
                      <img
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-violet-300 text-xs font-bold mt-1">{e.progress || 0}%</p>
                      </div>
                      <span className="text-slate-500 hover:text-violet-400 transition-colors text-sm shrink-0">→</span>
                    </motion.div>
                  ))}
                </div>
              ) : (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {achievements.map((a, i) => (
                <motion.div
                  ...
                  whileHover={a.earned ? { scale: 1.08, y: -3 } : {}}
                  className={`flex flex-col items-center text-center p-4 rounded-xl border transition-all cursor-default ${
                    a.earned ? `${a.bg} ${a.border}` : "bg-[#080410] border-[#2A1B4E] opacity-35 grayscale"
                  }`}
                >
        ...
        <div className="space-y-4">
          ...
          {enrollments.map((enrollment, i) => (
            <motion.div
              ...
              whileHover={{ x: 3 }}
              className="bg-[#120B24] p-5 rounded-2xl border border-[#2A1B4E] flex flex-col md:flex-row gap-5 hover:border-violet-500/30 transition-colors"
            >
              <div
                onClick={() => navigate(`/courses/${enrollment.course?._id}`)}
                className="w-full md:w-44 h-28 rounded-xl overflow-hidden shrink-0 cursor-pointer hover:opacity-90 transition-opacity"
              >
                <img src={enrollment.course?.thumbnail || "https://images.unsplash.com/photo-1517694712202-14dd9538aa97?w=600"} className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" alt="" />
              </div>
              <div className="flex-1 flex flex-col justify-center">
                <div className="flex items-start justify-between gap-4 mb-1">
                  <h3
                    onClick={() => navigate(`/courses/${enrollment.course?._id}`)}
                    className="text-lg font-bold text-white leading-tight cursor-pointer hover:text-violet-300 transition-colors"
                  >
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 bg-[#080410] rounded-full h-2 border border-[#2A1B4E] overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
                  ...
                  <span className="text-xs font-bold text-violet-300 shrink-0 w-10 text-right">{enrollment.progress || 0}%</span>
                  {(enrollment.progress || 0) === 100 ? (
                    <motion.button
                      whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.94 }}
                      onClick={() => setSelectedCert(enrollment)}
                      className="flex items-center gap-1.5 bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs px-3 py-1.5 rounded-lg font-bold hover:bg-amber-500/20 transition-colors shrink-0"
                    >
                    ...
                    <motion.button
                      whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.94 }}
                      onClick={() => navigate(`/courses/${enrollment.course?._id}`)}
                      className="bg-violet-600 hover:bg-violet-500 text-white text-xs px-4 py-1.5 rounded-lg font-bold shrink-0 transition-colors"
                    >
                      Continue
                    </motion.button>
                  )}
      ...
      {activeTab === "certificates" && (
        <div className="space-y-6">
          ...
          {!loadingEnrollments && enrollments.filter((e) => (e.progress || 0) === 100).length === 0 && (
            <div className="text-center py-16 bg-[#120B24] rounded-2xl border border-[#2A1B4E]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {enrollments.filter((e) => (e.progress || 0) === 100).map((enrollment, i) => {
              const dateStr = new Date(enrollment.completedAt || enrollment.enrolledAt)
                .toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" });
              const credId = `LA-${(enrollment._id || "").slice(-8).toUpperCase()}`;
              return (
                <motion.div
                  ...
                  className="relative bg-gradient-to-br from-violet-900/30 to-fuchsia-900/20 border border-violet-500/30 rounded-2xl p-7 overflow-hidden hover:border-violet-400/50 transition-all hover:shadow-[0_8px_40px_rgba(124,58,237,0.25)] group"
                >
                  ...
                  <div className="relative">
                    ...
                    <div className="flex items-center justify-between mb-5">
                      <div>
                        <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-1">Completed</p>
                        <p className="text-white font-bold text-sm">{dateStr}</p>
                      </div>
                      <div className="text-right">
